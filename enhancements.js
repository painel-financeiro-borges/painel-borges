...
<div class="card" style="flex:1;min-width:220px;margin-top:12px">
  ...
  <div id="reservas_progress_text" class="small">0%</div>
    ...
    <div class="card" id="dash_fluxo_card" style="margin-top:12px">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap">
        <div>
          <div class="small">Fluxo de Caixa — Mês</div>
          <div style="display:flex;gap:12px;align-items:center;margin-top:6px;flex-wrap:wrap">
            ...
            <div class="kv"><div class="small">Saídas</div><div id="dash_fluxo_exits" style="font-weight:800">R$ 0,00</div></div>
            <div class="kv"><div class="small">Consolidação</div><div id="dash_fluxo_net" style="font-weight:800">R$ 0,00</div></div>
          </div>
        </div>
    ...
    <div class="card" id="dash_plan_card" style="margin-top:12px">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap">
        <div>
          <div class="small">Planejamento Mensal — Progresso</div>
...
<div style="display:grid;grid-template-columns:1fr;gap:12px;margin-top:12px">
  <div class="card">
    <div class="small">Investimentos — total</div>
    ...
    <div class="card" style="margin-top:12px">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div><h3 style="margin:0">Últimas transações / lançamentos</h3><div class="small">Transações recentes</div></div>
  ...
  <div id="custo" class="page" style="display:none">
    <div class="card">
      ...
      <div class="small">Adicione seus custos mensais, por data e categoria. O gráfico mostra as categorias mais gastas.</div>
  ...
  <select id="cv_cat" class="input">
    <option>Moradia</option><option>Alimentação</option>
    <option>Transporte</option><option>Educação</option>
  ...
  <button class="btn-ghost" onclick="resetCusto()">🧹 Zerar Dados</button>
</div>
      ...
      <div style="margin-top:12px;display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
        <div style="flex:1">
          <table class="table" id="custoTable">
            ...
            <tbody><tr><td colspan="5" class="small">Sem lançamentos</td></tr></tbody>
          </table>
        </div>
        <div style="width:320px">
...
<div id="planejamento" class="page" style="display:none">
  <div class="card">
    ...
    <div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap">
      ...
      <button class="btn-ghost" onclick="resetPlanejamento()">🧹 Zerar Planejamento</button>
    </div>
  ...
  <div class="card">
    ...
    <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap">
      ...
      <select id="inv_categoria" class="input">
        <option>Renda Fixa</option><option>Ações</option><option>FIIs</option><option>Dólar</option>
        <option>Renda Fixa USA</option><option>Penny Stocks</option><option>Cripto</option>
      </select>
      ...
      <!-- BOTÃO DE RESET -->
      <button class="btn-ghost" onclick="resetInvestimentos()">🧹 Zerar Dados</button>
    </div>
    ...
    <div style="margin-top:12px;display:flex;gap:12px">
      <div style="flex:1">
        <table class="table" id="invTable">
          <thead>
            <tr>
              <th>Ativo</th><th>Categoria</th><th>Qtd</th><th>Preço</th><th>Valor</th><th>Ações</th>
            </tr>
          </thead>
          <tbody>
            <tr><td colspan="6" class="small">Nenhum investimento</td></tr>
          </tbody>
        </table>
      </div>
...
<div id="caderno" class="page" style="display:none">
  <div class="card">
    ...
    <div style="margin-top:10px;display:flex;gap:8px;align-items:center">
      ...
      <!-- BOTÃO DE RESET -->
      <button class="btn-ghost" onclick="resetCaderno()">🧹 Zerar Caderno</button>
...
<div id="backup" class="page" style="display:none">
  <div class="card">
    ...
    <div style="margin-top:20px;border-top:1px solid #ccc;padding-top:15px">
      ...
      <button class="btn" style="background:#d50000;color:#fff;font-weight:600" onclick="resetPainel()">🧹 Zerar Todos os Dados</button>
    </div>
...
<script>
/* ================= CONFIG ================= */
const API_URL = "REPLACE_WITH_YOUR_APPS_SCRIPT_URL"; // <--- atualize com seu Apps Script após deploy
...
function resetPlanejamento() {
  if (!confirm("Deseja zerar TODO o planejamento mensal?")) return;
  state.planejamento = {};
  saveLocal();
}

function resetFluxo() {
  if (!confirm("Deseja zerar TODOS os lançamentos do Fluxo Diário?")) return;
  state.fluxo = [];
  saveLocal();
}
...
function resetInvestimentos() {
  if (!confirm("Deseja zerar TODOS os investimentos?")) return;
  state.investimentos = [];
  saveLocal();
}

function resetCaderno() {
  if (!confirm("Deseja zerar TODO o Caderno de Notas?")) return;
  state.caderno = [];
  saveLocal();
}
...
function loadLocal(){
  try{
    ...
    if(raw){
      ...
      // garantir que settings tenha todas as chaves modernas
...
function formatMoney(v, tipo){
  ...
  // 1) OCULTAÇÃO MASTER
  if (state.settings.hideAll) {
    return 'R$ ••••';
  }

  // 2) OCULTAÇÃO POR TIPO ESPECÍFICO
...
function addPlanItem(monthKey, pid){ const desc = prompt('Descrição da conta (ex: Consórcio)'); if(!desc) return; let input = prompt('Valor (R$)','0');
if (!input) return;
...
function addBlock(){ const title = el('#note_block_title').value.trim(); if(!title) return alert('Insira título do bloco'); const activeEl = document.querySelector('.note-tab.active'); if(!activeEl) return alert('Selecione uma aba primeiro'); const tab = state.caderno.find(t=>t.id===activeEl.dataset.id); if(!tab) return; tab.blocks.push({ id: uid('b'), title, items: [] }); el('#note_block_title').value=''; saveLocal(); renderNoteBlocks(tab.id); }
...
async function postToDrive(payload){
  if(!API_URL || API_URL.includes('REPLACE_WITH')) return alert('API_URL não configurada. Atualize o campo API_URL no código com a URL do Apps Script.');
  try{
...
async function syncAllToDrive(){
  if(!confirm('Sincronizar TODO o painel com Drive (sobrescreverá as abas no Drive)?')) return;
  ...
  alert('Sincronização solicitada. Verifique a planilha no Drive.');
}
