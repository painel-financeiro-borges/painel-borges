...
div.innerHTML = `<div class="w-100"><div class="d-flex align-items-center"><i class="fas fa-grip-vertical checklist-handle me-2"></i><input type="checkbox" ${item.done ? 'checked' : ''} onchange="window.toggleTempItem(${index})"><span class="ms-2">${item.text} ${tagHtml}</span> <div class="d-flex gap-2 ms-auto">${btnNote}<i class="fas fa-share text-primary" style="cursor:pointer" onclick="window.moveTempItemPrompt(${index})" title="Mover"></i><i class="fas fa-pen text-secondary" style="cursor:pointer" onclick="window.editTempItem(${index})" title="Editar"></i><i class="fas fa-times text-danger" style="cursor:pointer" onclick="window.removeTempItem(${index})" title="Excluir"></i></div></div>${noteHtml}</div>`;
