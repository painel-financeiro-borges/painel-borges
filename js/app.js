...
function initSubCards(projectId) {
    ...
    subCardUnsub = onSnapshot(q, (snap) => {
        ...
        snap.forEach(docSnap => {
            ...
            const el = document.createElement('div'); el.className = `sub-card ${data.color || 'bg-grad-1'}`;
...
function initKanban(projectId) {
    ...
    kanbanUnsub = onSnapshot(q, (snap) => {
        ...
        snap.forEach(docSnap => {
            ...
            const el = document.createElement('div'); el.className = `task-card priority-${p}`; el.dataset.id = docSnap.id; el.onclick = () => editTask(docSnap.id, t);
